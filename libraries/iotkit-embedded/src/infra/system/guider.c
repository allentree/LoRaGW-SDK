...
static char g_mqtt_url[256];
static uint32_t g_mqtt_port = 0;
static int g_use_pk = 0;
int _get_conf_mqtt_url() {
    FILE *fp = NULL;
    char buff[256] = {0};
    char mqtt_url[256] = {0};
    ...
    g_mqtt_port = 0;
    memset(g_mqtt_url, 0x0, sizeof(g_mqtt_url));

    fp = fopen("mqtt-conn.ini", "rb");
    if (NULL == fp) {
        sys_info("fopen mqtt-conn.ini file failed\n");
        return -1;
    }

    memset(buff, 0x0, sizeof(buff));
    fgets(buff, sizeof(buff), fp);
    sys_info("buff:%s", buff);
    fclose(fp);
    fp = NULL;

    memset(mqtt_url, 0x0, sizeof(mqtt_url));
    sscanf(buff, "%[^:]:%u", mqtt_url, &mqtt_port);
    if ((strlen(mqtt_url) <= 0) || (mqtt_port <= 0)) {
        sys_info("the conf url or port invalid\n");
        return -1; 
    }

    g_mqtt_port = mqtt_port;
    if ((strlen(mqtt_url) > 5) && (strncmp(mqtt_url, "{pk}.", 5) == 0)) {
        g_use_pk = 1;
        strcpy(g_mqtt_url, mqtt_url + 5);
    } else if (strlen(mqtt_url) > 0) {
        g_use_pk = 0;
        strcpy(g_mqtt_url, mqtt_url);
    }
    sys_info("conf url:%s", g_mqtt_url);
    sys_info("conf port:%u", g_mqtt_port);
    sys_info("conf use pk:%d", g_use_pk);
...
int iotx_guider_authenticate(iotx_conn_info_t *conn)
{
    ...
    /* Begin add for Edge CN */
    _get_conf_mqtt_url();
    if ((strlen(g_mqtt_url) > 0) && (g_mqtt_port > 0)) {
        conn->port = g_mqtt_port;
        len = strlen(dev.product_key) + 2 + strlen(g_mqtt_url);
        conn->host_name = SYS_GUIDER_MALLOC(len);
        if (conn->host_name == NULL) {
            goto failed;
        }
