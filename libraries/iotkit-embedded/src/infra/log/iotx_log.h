...
#define COL_WHE "\x1B[37m"
#define COL_CYN "\x1B[36m"
#define COL_MAG "\x1B[35m"

typedef const char cchar;

extern void log_print(LOG_LEVEL lvl, cchar *color, cchar *t, cchar *f, cchar *func, int l, cchar *fmt, ...);

#define log_i(tag, args...) \
        log_print(LOG_LEVEL_INFO,COL_GRE,tag,__FILE__,__FUNCTION__,__LINE__,##args)
#define log_w(tag, args...) \
        log_print(LOG_LEVEL_WARN,COL_CYN,tag,__FILE__,__FUNCTION__,__LINE__,##args)
#define log_e(tag, args...) \
        log_print(LOG_LEVEL_ERR,COL_YEL,tag,__FILE__,__FUNCTION__,__LINE__,##args)
#define log_f(tag, args...) \
        log_print(LOG_LEVEL_FATAL,COL_RED,tag,__FILE__,__FUNCTION__,__LINE__,##args)
#define log_d(tag, args...) \
        log_print(LOG_LEVEL_DEBUG,COL_WHE,tag,__FILE__,__FUNCTION__,__LINE__,##args)

#define log_emerg(mod, ...)     log_f(mod, ##__VA_ARGS__);
#define log_crit(mod, ...)      log_f(mod, ##__VA_ARGS__);
#define log_err(mod, ...)       log_e(mod, ##__VA_ARGS__);
#define log_warning(mod, ...)   log_w(mod, ##__VA_ARGS__);
#define log_info(mod, ...)      log_i(mod, ##__VA_ARGS__);
#define log_debug(mod, ...)     log_d(mod, ##__VA_ARGS__);
#define log_flow(mod, ...)      log_d(mod, ##__VA_ARGS__);
#else
