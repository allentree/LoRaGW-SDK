...
int ws_nopoll_write(void *handle, const char *buffer, int len, int timeout_ms)
{
    ...
    if ((NULL != buffer) && (len > 0)) {
        ...
        LOGD ("send len: %d, try: %d.\n", bytes_written, tries);
        return  bytes_written;
    } else {
        LOGD ("ERROR: content is NULL or len <= 0\n");
    }
    return -1;
}
...
noPollConn *ws_nopoll_connect(const int debug_on, const char *host, const char *port, const char *get_url, const int tls_on, const char *ca_crt)
{
    ...
    if (nopoll_ctx_conns(ctx) != 0) {
        LOGD("ERROR: expected to find 0 registered connections but found\n");
        return NULL;
    }
    ...
    if (1 == tls_on) {
        ...
        if (!nopoll_conn_is_ok (conn)) {
            LOGD("ERROR: Expected to find proper client connection status, but found error..\n");
            nopoll_ctx_unref(ctx);
            return NULL;
        }

        if (!nopoll_conn_is_tls_on (conn)) {
            LOGD("ERROR: expected to find TLS enabled on the connection but found it isn't..\n");
            nopoll_ctx_unref(ctx);
            return NULL;
        }    
    } else {
    ...
    while (nopoll_true) {
        ...
        i++;
        if (i > 100) {
            LOGD("ERROR: connection not ready\n");
            nopoll_conn_close(conn);
            nopoll_ctx_unref(ctx);
            return NULL;
        }
        LOGD ("conn not ready yet..\n");
        nopoll_sleep (100000);
    }
...
noPollConn *ws_nopoll_listener(const int debug_on, const char *host, const char *port, const int tls_on, const char *server_crt, const char *private_key)
{
    ...
    if (1 == tls_on) {
    	...
    	if (!nopoll_conn_is_ok (listener)) {
    		LOGD("ERROR: Expected to find proper listener TLS connection status, but found..\n");
    		nopoll_ctx_unref(ctx);
    		return NULL;
    	}
    	...
    	if (!nopoll_listener_set_certificate (listener, server_crt, private_key, NULL)) {
    		LOGD("ERROR: unable to configure certificates for TLS websocket..\n");
    	...
    	if (! nopoll_ctx_set_certificate (ctx, NULL, server_crt, private_key, NULL)) {
    		LOGD("ERROR: unable to setup certificates at context level..\n");
            nopoll_conn_close (listener);
            nopoll_ctx_unref(ctx);
    		return NULL;
    	}
        ...
        if (!nopoll_conn_is_ok (listener)) {
            LOGD ("ERROR: Expected to find proper listener connection status, but found..\n");
    		nopoll_ctx_unref(ctx);
            return NULL;
        }
	}
...
int ws_nopoll_loop_wait(void *handle)
{
    ...
    if (NULL == conn) {
        LOGD("ERROR: conn is NULL\n");
        return -1;
    }

    ctx = nopoll_conn_ctx(conn);
    if (NULL == ctx) {
        LOGD("ERROR: ctx is NULL\n");
        return -1;
    }
...
noPollConn *ws_nopoll_accept(void *handle)
{
    ...
    if (NULL == master) {
        LOGD("ERROR: master conn is NULL\n");
        return NULL;
    }
	...
	if (!nopoll_conn_is_ok (conn)) {
		LOGD("ERROR: expected to find proper listener status (connection accepted), but found failure..\n");
		return NULL;
	}
