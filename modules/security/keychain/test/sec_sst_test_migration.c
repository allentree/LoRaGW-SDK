...
{
    char *file_path = "/tmp/data/test_enc_dec_short";
    char *key = NULL;
    uint32_t key_len = 0;
    ...
    ret = kc_encrypt_data(data, strlen(data), file_path, key, &key_len);
    ...
    ret = kc_decrypt_data(file_path, key, key_len, dec_data, &dec_len);
    if (ret != KC_ERROR_SHORT_BUFFER) {
...
{
    char *file_path = "/tmp/data/test_enc_dec";
...
{
    char *file_path = "/tmp/data_test1/test1/test_enc_folder_parent_not_exist";
    ...
    ret = kc_encrypt_data(data, strlen(data), file_path, key, &key_len);
    if (ret != KC_ERROR_BAD_PARAMETERS) {
...
{
    char *file_path = "/tmp/data1/test_enc_folder_not_exist";
...
{
    char *file_path = "/tmp/data/test_dec_not_exist";
    ...
    file_path = "/tmp/data/test_enc_dec_not_exist_1";
    ret = kc_decrypt_data(file_path, key, key_len, dec_data, &dec_len);
