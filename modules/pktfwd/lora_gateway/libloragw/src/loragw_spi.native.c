...
int lgw_spi_open(void **spi_target_ptr,int index) {
	...
	if(XS508_Check(VID,PID,index) != 0){
    	DEBUG_MSG("ERROR verify key from EEPROM\n");
	...
	struct mpsse_context *mpsse = OpenIndex(VID,PID,SPI0, SIX_MHZ, MSB, IFACE_A, NULL, NULL, index);
	if (mpsse == NULL) {
